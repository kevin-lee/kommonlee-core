 ...
 */
public final class LongTotal
{
  private LongTotal() throws IllegalAccessException
  {
  ...
  public static <T> long total(final T[] numberSource, final Function1<T, Long> longMapper)
  {
    long total = 0;
    for (final T each : numberSource)
    {
      @SuppressWarnings("boxing")
      final long l = longMapper.apply(each);
      total += l;
    }
    return total;
  }
  ...
  public static <T> long total(final Iterable<T> numberSource, final Function1<T, Long> longMapper)
  {
    long total = 0;
    for (final T each : numberSource)
    {
